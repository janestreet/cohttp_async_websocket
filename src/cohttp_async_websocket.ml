...
module Client = struct
  ...
  let create ?force_ssl_overriding_SNI_hostname ?headers ?(opcode = `Text) uri =
  ...
  let with_websocket_client ?headers ?(opcode = `Text) uri ~f =
    match%bind create ?headers ~opcode uri with
